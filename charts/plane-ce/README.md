...
| ingress.ingress_annotations | `{ "nginx.ingress.kubernetes.io/proxy-buffer-size": "8k" }` |          | Annotations applied to the standard `Ingress` resource. Defaults to a `proxy-buffer-size` of `8k` so large upstream response headers (e.g. the Swagger UI / OpenAPI schema endpoints) are not truncated. **Only used when `ingressClass` is not `traefik`.** When Traefik is selected, use `ingress.traefik.maxRequestBodyBytes` to control request body size instead. |
