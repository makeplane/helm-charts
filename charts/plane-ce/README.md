...
>
> If you must stage them, pin `rabbitmq.image` to `rabbitmq:3.13.6-management-alpine` when taking this chart, upgrade
> `planeVersion`, then remove the pin.
>
> **The chart enforces this.** `rabbitmq.minPlaneVersion` is the lowest `planeVersion` known to work with a 4.x
...
| minio.image | docker.io/pgsty/minio:RELEASE.2026-08-04T00-00-00Z | | Using this key, user must provide the docker image name to setup the stateful deployment of `minio`. (must be set when `minio.local_setup=true`) |
| minio.image_mc | docker.io/pgsty/mc:RELEASE.2026-09-16T00-00-00Z | | Using this key, user must provide the docker image name to setup the job deployment of `minio client`. (must be set when `minio.local_setup=true`) |
...
| ssl.externalTermination     |                           false                           |          | Set `true` when TLS is terminated **in front of** Plane and this chart manages no certificate — a cloud load balancer, Cloudflare, a service mesh, or a Traefik entrypoint carrying its own cert. Renders `WEB_URL` as `https://` and emits **no** `tls:` block. Does not move the Traefik entrypoint; see [TLS options](#tls-options-choosing-how-https-is-handled), option 4a/4b. |
