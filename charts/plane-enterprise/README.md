...
Runs the [Plane MCP server](https://github.com/makeplane/plane-mcp-server) inside the release and routes `services.mcp_server.path_prefix` on the Plane host to it. Register an OAuth application in Plane (workspace settings > Integrations) with redirect URIs `https://<licenseDomain>/mcp/http/auth/callback` and `https://<licenseDomain>/mcp/auth/callback`; the server does not start without `plane_oauth.client_id`.
...
| services.mcp_server.plane_oauth.enabled |                  false                   |          | Set to `true` to hand the MCP server a Plane OAuth application so it can authenticate MCP clients against this instance.                                                                        |
| services.mcp_server.plane_oauth.client_id |                 ""                     | required if `services.mcp_server.plane_oauth.enabled` is `true` | Plane OAuth client ID (`PLANE_OAUTH_PROVIDER_CLIENT_ID`); stored in the MCP server Secret                                                                     |
| services.mcp_server.plane_oauth.client_secret |               ""                   | required if `services.mcp_server.plane_oauth.enabled` is `true` | Plane OAuth client secret (`PLANE_OAUTH_PROVIDER_CLIENT_SECRET`); stored in the MCP server Secret                                                             |
| services.mcp_server.plane_oauth.provider_base_url |             ""                 | required if `services.mcp_server.plane_oauth.enabled` is `true` | OAuth provider base URL (`PLANE_OAUTH_PROVIDER_BASE_URL`), normally the Plane instance URL                                                                    |
...
|                          | `PLANE_OAUTH_PROVIDER_CLIENT_ID` | Yes (if `services.mcp_server.plane_oauth.enabled=true`)    | Plane OAuth application client ID                | `<client-id>`                                                                                                                                                                                        |
|                          | `PLANE_OAUTH_PROVIDER_CLIENT_SECRET` | Yes (if `services.mcp_server.plane_oauth.enabled=true`) | Plane OAuth application client secret          | `<client-secret>`                                                                                                                                                                                    |
|                          | `PLANE_OAUTH_PROVIDER_BASE_URL` | Yes (if `services.mcp_server.plane_oauth.enabled=true`)      | OAuth provider base URL (the Plane instance URL) | `https://plane.example.com`                                                                                                                                                                          |
...
| plane.example.com       |   /mcp/\*    | <http://plane-app-mcp-server.plane:8211> | Yes (if `services.mcp_server.enabled=true`; path follows `services.mcp_server.path_prefix`) |
| plane.example.com       | /.well-known/oauth-protected-resource/mcp\* | <http://plane-app-mcp-server.plane:8211> | Yes (if `services.mcp_server.enabled=true`; OAuth discovery, path-inserted at the host root by MCP clients) |
| plane.example.com       | /.well-known/oauth-authorization-server/mcp\* | <http://plane-app-mcp-server.plane:8211> | Yes (if `services.mcp_server.enabled=true`; OAuth discovery, path-inserted at the host root by MCP clients) |
