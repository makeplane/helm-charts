...
Self-hosted [draw.io](https://www.drawio.com/) for the Wiki editor's diagram extension. Like the other bundled services it is gated by `services.drawio.local_setup` (default `false`). With `local_setup: true` the nginx, Traefik and OpenShift ingress templates serve it on the `/drawio` subpath of the license domain and `DRAWIO_EMBED_URL` is set to `<scheme>://<licenseDomain>/drawio`. With `local_setup: false`, `DRAWIO_EMBED_URL` comes from `env.drawio_remote_url`, which defaults to the public diagrams.net cloud (`https://embed.diagrams.net`). The image serves its webapp at `/` and loads its assets relatively, so the ingress strips the `/drawio` prefix and (on nginx and Traefik) redirects a bare `/drawio` to `/drawio/`. On nginx this uses separate `<release>-drawio` Ingress objects with `rewrite-target`, which switches ingress-nginx to regex path matching for the whole host. `makeplane/drawio` is versioned independently of `planeVersion`.
...
| env.drawio_remote_url                    | https://embed.diagrams.net |          | drawio endpoint rendered as `DRAWIO_EMBED_URL` when `services.drawio.local_setup=false` (ignored when `true`). Defaults to the public diagrams.net cloud; point it at a drawio hosted outside this chart to self-host |
