...
| services.mcp_server.image           |          makeplane/plane-mcp-server          |          | Docker image for the MCP server.                                                                                                                                                                |
| services.mcp_server.tag             |                    v0.3.0                    |          | Image tag for the MCP server. Unlike the other services this is not tied to `planeVersion`, because the MCP server is released on its own cadence.                                              |
...
| mcp_server_env_existingSecret | `REDIS_HOST` / `REDIS_PORT` | Yes (if `services.mcp_server.enabled=true` and `external_secrets.redis.secretName` is unset) | Redis endpoint for the MCP server's session store | `plane-redis.plane-ns.svc.cluster.local` / `6379`                                                                                                          |
|                          | `REDIS_PASSWORD` / `REDIS_SSL` | No                                                           | Redis auth token and TLS flag (`true`) when the remote Redis needs them | `password` / `true`                                                                                                                                  |
