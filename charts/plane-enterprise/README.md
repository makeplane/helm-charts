...
| env.drawio_embed_url           |                      ""                         |          | (optional) drawio endpoint for the Wiki editor's diagram extension (`DRAWIO_EMBED_URL`). Leave empty to use the in-chart drawio at `<scheme>://<licenseDomain>/drawio` when `services.drawio.enabled=true`, or the diagrams.net cloud otherwise. Set it to use a drawio hosted outside this chart. |
...
Self-hosted [draw.io](https://www.drawio.com/) for the Wiki editor's diagram extension, off by default (the editor then uses the diagrams.net cloud). When enabled, the nginx, Traefik and OpenShift ingress templates serve it on the `/drawio` subpath of the license domain, and `DRAWIO_EMBED_URL` is set to `<scheme>://<licenseDomain>/drawio` unless `env.drawio_embed_url` overrides it. The image serves its webapp at `/` and loads its assets relatively, so the ingress strips the `/drawio` prefix and (on nginx and Traefik) redirects a bare `/drawio` to `/drawio/`. On nginx this uses separate `<release>-drawio` Ingress objects with `rewrite-target`, which switches ingress-nginx to regex path matching for the whole host. `makeplane/drawio` is versioned independently of `planeVersion`.

| Setting                                  |         Default          | Required | Description                                                                                                                                                                             |
| ---------------------------------------- | :----------------------: | :------: | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| services.drawio.enabled                  |          false           |          | Set to `true` to deploy draw.io and route `/drawio` to it                                                                                                                               |
...
| plane.example.com       |  /drawio/\*  | <http://plane-app-drawio.plane:8080>    | Yes (if `services.drawio.enabled=true`; `/drawio` prefix stripped)          |
