...
| services.pi_worker.queueName        |   ''    |          | Which Celery queue(s) the Plane AI (PI) worker consumes, comma-separated. Empty means both `plane_pi_queue` and `plane_pi_agent_queue`, or just `plane_pi_queue` when `pi_agent_worker` is enabled. Set it to override.     |
| services.pi_agent_worker.enabled    |  false  |          | Runs a dedicated worker for native agent runs on `plane_pi_agent_queue`, so an agent run never queues behind a long vectorization task. This is added alongside `pi_worker`, which must keep running.                        |
| services.pi_agent_worker.replicas   |    1    |          | Number of replicas for the Plane AI (PI) agent worker deployment.                                                                                                                                                           |
| services.pi_agent_worker.queueName  | plane_pi_agent_queue |  | The queue the agent worker consumes.                                                                                                                                                                             |
| services.pi_agent_worker.concurrency|    4    |          | Celery worker processes for agent runs. Agent runs are I/O-bound on streaming LLM calls, so this can be higher than the general worker's.                                                                                    |
| services.pi_agent_worker.maxTasksPerChild | 100 |      | Recycles a worker child after this many tasks. Each agent run builds its own event loop and database pools, so a lower number than the default 1000 bounds their growth.                                                     |
| services.pi_agent_worker.memoryLimit | 1000Mi |         | Memory limit for the Plane AI (PI) agent worker deployment.                                                                                                                                                                 |
| services.pi_agent_worker.cpuLimit   |  500m   |          | CPU limit for the Plane AI (PI) agent worker deployment.                                                                                                                                                                    |
| services.pi_agent_worker.memoryRequest | 256Mi |        | Memory request for the Plane AI (PI) agent worker deployment.                                                                                                                                                               |
| services.pi_agent_worker.cpuRequest |  100m   |          | CPU request for the Plane AI (PI) agent worker deployment.                                                                                                                                                                  |
