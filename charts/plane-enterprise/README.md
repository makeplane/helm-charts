...
| services.silo.init_image                      |                   busybox                    |                                                                 | Using this key, user can provide the docker image used by the `silo` init containers, which wait for RabbitMQ to become resolvable and (when a custom S3 CA is configured) assemble the CA bundle                |
