        ...
        for field in ("stringData", "data"):
            for k, v in (doc.get(field) or {}).items():
                if not isinstance(v, str):
                    continue
                if field == "data":
                    # Secret `data` is base64. Yielding it raw would make every content
                    # check — the DSN scan above all — look at gibberish and pass.
                    try:
                        v = base64.b64decode(v, validate=True).decode("utf-8")
